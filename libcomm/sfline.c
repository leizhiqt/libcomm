...
int save_txt(const char *file_name, const char *buf) {
	...
	if (fd == -1)
	{
		LOG_INFO(" open:%s RET:%d", file_name, RET);
		return -2;
	}

	RET = write(fd, buf, strlen(buf));
	LOG_INFO(" write:%s RET:%d", file_name, RET);

	//release:
	RET = close(fd);
	LOG_INFO("close:%s RET:%d", file_name, RET);
...
void send_email(const char *buf, const char *subject, const char *attach, const char *to_addr)
{
	...
	LOG_INFO("send_email:%s", send_email);
	system(send_email);
}
